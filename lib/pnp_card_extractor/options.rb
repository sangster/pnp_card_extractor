...
module PnpCardExtractor
  ...
  class Options
    ...
    DEFAULTS = {
      card_order: '-',
      pages_str: '2-',
      filename_template: ['{cycle.position} {cycle.name}',
                          '{pack.position} {pack.name}',
                          '{code}{is_variant ? "-"}{is_variant ? variant_position} {stripped_title}.png'
                         ].join('/').freeze,
      extra_template: ['{cycle.position} {cycle.name}',
                       '{pack.position} {pack.name}',
                       'extra-cards',
    ...
    def logger
      @logger ||= self[:silent] ? Logging.fake_logger : build_logger
    end
    ...
    def api
      @api ||= Db::NetrunnerdbApi.new(fetch(:api_uri, DEFAULTS[:api_uri]), logger:)
    end
    ...
    def parser
      @parser ||= OptionParser.new("Usage: #{$0} [OPTIONS] PDF_FILE") do |op|
        op.separator "\nOPTIONS\n\n"
        op.on('-p', '--pages=NUMBERS',
              "The pages to extract cards from. See NUMBERS section. #{def_str :pages_str}") do |str|
          values[:pages_str] = str
        end
        op.on('-f', '--force', 'Overwrite existing files.') { values[:force] = true }
        op.on('-d', '--directory=DIRECTORY',
              'Directory to export cards into. Will be ignored if --filename-template ',
              "defines an absolute path. #{def_str :directory}") do |str|
          values[:directory] = str
        end
        op.on('-m', '--margins=MARGINS',
              'Use the given margins ("top,right,bottom,left"), instead of measuring.') do |str|
        ...
        op.on('-M', '--measure-all-margins',
              'Instead of assuming they\'re all equal, measure each margin on every page.') do
        ...
        op.on('-o', '--card-order=NUMBERS',
              'The card numbers found in the PDF. See NUMBERS and CARD ORDER sections.',
        ...
        op.on('-C', '--cache-directory=CODE',
              ...
              '(Default: $XDG_CACHE_HOME/pnp_card_extractor/ or ~/.cache/pnp_card_extractor/)') do |str|
    ...
    def build_logger
      ...
      Logging.wrap_logger(Logger.new($stderr, level:).tap do |log|
        log.formatter = proc do |sev, time, name, msg|
          ...
          sev = sev[...5].ljust(5)
          name = name.to_s.split('::', 2).last
          [time, sev, name].join('  ').concat(": #{msg}\n")
        end
      end)
    end
  end
